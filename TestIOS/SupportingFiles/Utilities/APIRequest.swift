    ...
    static func call(
            ...
            failure(
                APIError(
                    message: NSLocalizedString(
                        "No Internet connection", comment: ""), status: 0))
            return
        }
        ...
        guard let url = buildURL(path: path, queryItems: queryItems) else {
            failure(
                APIError(
                    message: NSLocalizedString(
                        "Failed to create URL", comment: ""), status: 0))
            return
        }
        ...
        if let parameters = parameters {
            do {
                ...
                failure(
                    APIError(
                        message: NSLocalizedString(
                            "Error encoding request parameters", comment: ""),
                        status: 0))
                return
            }
        }
        ...
        let task = URLSession.shared.dataTask(
            ...
            if error != nil {
                failure(
                    APIError(
                        message: NSLocalizedString(
                            "Failed to get a valid response", comment: ""),
                        status: 0))
                return
            }
            ...
            guard let data = data else {
                failure(
                    APIError(
                        message: NSLocalizedString(
                            "No data received from the server", comment: ""),
                        status: httpResponse.statusCode))
                return
            }
