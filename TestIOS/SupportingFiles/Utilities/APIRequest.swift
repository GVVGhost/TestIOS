...
class APIRequest<Parameters: Encodable, Model: Decodable> {
    static func call(
        ...
        if let parameters = parameters {
            do {
                ...
                failure(APIError(message: NSLocalizedString("Error encoding request parameters", comment: ""), status: 0))
                return
            }
        }
        ...
        let task = URLSession.shared.dataTask(with: request) { data, response, error in
            ...
            guard (200...299).contains(httpResponse.statusCode) else {
                failure(APIError(message: NSLocalizedString("Server error", comment: ""), status: httpResponse.statusCode))
                return
            }
            guard let data = data else {
                failure(APIError(message: NSLocalizedString("No data received from the server", comment: ""), status: httpResponse.statusCode))
                return
            }
            completion(data)
        }
    ...
    private static func buildURL(path: String, queryItems: [URLQueryItem]?) -> URL? {
        let scheme: String? =
            Bundle.main.infoDictionary?["API_SCHEME"] as? String
